...
describe("japlan what do you know about me", () => {
  ...
  it("not finished: says so and offers the next question, instead of 'i know nothing'", async () => {
    ...
    expect(lastIn(dm("sam"))).toBe(
      "you haven't finished the quick questions yet, so i only know the basics. want to keep going? next one: insane local food spot you've never heard of, or kayaking somewhere stupidly pretty?",
    );
    // The next DM answers that question.
    expect(h.db.table("participants").find((p) => p.id === "p-sam")!.survey_state).toBe("ab_food_outdoors");
  });
});
