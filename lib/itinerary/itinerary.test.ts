...
describe("itinerary draft boundaries", () => {
  it("uses inclusive calendar days without a UTC off-by-one", () => {
    ...
    expect(dates).toHaveLength(Math.round((Date.parse(`${developmentTripConfig.endDate}T00:00:00Z`) - Date.parse(`${developmentTripConfig.startDate}T00:00:00Z`)) / 86_400_000) + 1);
  });
  ...
  it("uses the model only to choose research candidates", async () => { const candidates = mockResearch(developmentTripConfig).candidates; const provider: LLMProvider = { complete: async () => JSON.stringify({ days: inclusiveTripDates(developmentTripConfig).map((date, i) => ({ date, dayNumber: i + 1, summary: "draft", activities: [{ candidateActivityId: candidates[i % candidates.length].id, startTime: "10:00", endTime: "11:00", notes: "unvalidated draft" }] })) }) }; const itinerary = await generateDraftItinerary(developmentTripConfig, candidates, provider); expect(itinerary.days).toHaveLength(inclusiveTripDates(developmentTripConfig).length); expect(itinerary.days.flatMap(day => day.activities).every(activity => candidates.some(candidate => candidate.id === activity.id))).toBe(true); });
  it("keeps explicit mock generation deterministic and draft-only", () => { const draft = generateMockDraftItinerary(developmentTripConfig, mockResearch(developmentTripConfig).candidates); expect(draft.status).toBe("draft"); expect(draft.days).toHaveLength(inclusiveTripDates(developmentTripConfig).length); expect(draft.days.every(day => day.activities[0]?.verificationStatus === "unvalidated")).toBe(true); });
});
