...
export const CandidateActivitySchema = z.object({
  ...
  sourceUrls: z.array(z.string().url()).min(1), unverifiedFields: z.array(z.string()),
});
...
export const ItinerarySelectionSchema = z.object({ candidateActivityId: z.string(), startTime: z.string().regex(/^([01]\d|2[0-3]):[0-5]\d$/).nullable(), endTime: z.string().regex(/^([01]\d|2[0-3]):[0-5]\d$/).nullable(), notes: z.string().max(500) });
export const ItineraryModelSchema = z.object({ days: z.array(z.object({ date: z.string().regex(/^\d{4}-\d{2}-\d{2}$/), dayNumber: z.number().int().positive(), summary: z.string().min(1), activities: z.array(ItinerarySelectionSchema) })) });
export type ItineraryModel = z.infer<typeof ItineraryModelSchema>;

export type DraftItinerary = { destination: string; startDate: string; endDate: string; status: "draft"; days: Array<{ date: string; dayNumber: number; summary: string; activities: Array<CandidateActivity & { startTime: string | null; endTime: string | null; notes: string; verificationStatus: "unvalidated" }> }> };
