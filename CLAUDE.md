...
- Generation asks for boldness by count (`boldTasksWanted`: chill 1, normal 2, unhinged all) and shows the model every title already on the trip. Each task has a `kind` and a `place` (generation-time only, not stored); `enforceBoardMix` drops a second task at the same place and an all-one-kind board, which the normal regenerate/fallback path then refills.
