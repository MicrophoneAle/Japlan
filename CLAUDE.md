...
- **Boards on demand.** "japlan plans / tomorrow / day 3 / friday" (`isBoardRequest`, `parseBoardDay`) runs `answerBoardRequest` (`lib/handlers/board-request.ts`): lists the day's open tasks, or builds the board now through `buildBoardForDate`, the one pipeline the cron also uses. `boards` (one row per trip-day, inserted before generating: the lock) tracks provisional and delivery; `board_requests` (unique per person per local day) is the one-on-demand-generation-per-day guard. Future days are provisional and regenerate on their morning unless something on them was claimed. Claimed tasks are never overwritten.
